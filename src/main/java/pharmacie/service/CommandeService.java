...
public class CommandeService {
    ...
    public Ligne ajouterLigne(int commandeNum, int medicamentRef, @Positive int quantite) {
        ...
        if (medicament.getUnitesEnStock() < quantite) {
            throw new IllegalStateException("Pas assez de stock pour le médicament demandé");
        }
        // Vérifier si le médicament est déjà présent dans la commande
        for (Ligne ligne : commande.getLignes()) {
            if (ligne.getMedicament().getReference().equals(medicamentRef)) {
                // Le médicament est déjà présent, on additionne les quantités
        ...
        // Le médicament n'est pas encore présent dans la commande : on crée une ligne
        Ligne nouvelleLigne = new Ligne(commande, medicament, quantite);
        // maintenir la relation bidirectionnelle
        commande.getLignes().add(nouvelleLigne);
        medicament.setUnitesCommandees(medicament.getUnitesCommandees() + quantite);
        Ligne saved = ligneDao.save(nouvelleLigne);
        medicamentDao.save(medicament);
        return saved;
    }
